 ...
 */
angular.module('controlPanelApp').controller('BusMovementReportController', BusMovementReportController);

function BusMovementReportController(ReportService, $scope, $state, $localStorage, $filter, MonitoringService, $timeout, $rootScope, Constants, MiscServices) {
    ...
    function loadData(tableState) {
        ...
        ReportService.getVehicleMovement(filter).then(
            function (response) {
    ...
    self.reportFileName = function (fileType) {
        return MiscServices.generateReportFileName('BusMovement', fileType);
    }
    ...
    self.downloadFile = function (reportType) {
        ...
        var urlSuffix = 'restapi/report/vehicle/getVehicleMovement/download.json';
